...
const checkEnd = function (board, x, y) {
  if (
    checkColumnsPoint(board, x, y) ||
    checkRowsPoint(board, x, y) ||
    checkDiagonalsPoint(board, x, y)
  )
    return true;
};
...
const minimax_depth = function (evalBoard, depth, player) {
  ...
  if (depth < maxDepth) {
    ...
    for (let i = 0; i < maxMove; i++) {
      var move = {};
      move = maxPos(score).pos;
      // console.log(move)
      moves.push(move);
      score[move.y][move.x] = 0;
    }
    const values = [];
